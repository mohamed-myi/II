...
def upgrade() -> None:
    ...
    with op.batch_alter_table('repository', schema='ingestion') as batch_op:
        ...
        batch_op.create_index(batch_op.f('ix_ingestion_repository_primary_language'), ['primary_language'], unique=False)
    ...
    with op.batch_alter_table('issue', schema='ingestion') as batch_op:
        batch_op.create_index(batch_op.f('ix_ingestion_issue_heat_score'), ['heat_score'], unique=False)
        batch_op.create_index(batch_op.f('ix_ingestion_issue_ingested_at'), ['ingested_at'], unique=False)

    op.create_table('bookmarkedissue',
    sa.Column('id', sa.Uuid(), nullable=False),
    ...
    sa.Column('user_id', sa.Uuid(), nullable=False),
    sa.Column('history_vector', pgvector.sqlalchemy.vector.VECTOR(dim=256), nullable=True),
    sa.Column('intent_vector', pgvector.sqlalchemy.vector.VECTOR(dim=256), nullable=True),
    ...
    sa.Column('raw_intent_text', sqlmodel.sql.sqltypes.AutoString(), nullable=False),
    ...
    sa.Column('updated_at', sa.DateTime(), nullable=False),
    sa.ForeignKeyConstraint(['bookmark_id'], ['public.bookmarkedissue.id'], ),
    sa.PrimaryKeyConstraint('id'),
    schema='public'
    )
